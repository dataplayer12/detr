...
def get_args_parser():
    ...
    parser.add_argument("--mlflow_runname", type=str, help="Name of mlflow run")
...
def main(args):
    ...
    mlflow.set_experiment("detr_training")
    mlflow.start_run(run_name=args.mlflow_runname)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            for checkpoint_path in checkpoint_paths:
                ...
                mlflow.pytorch.log_state_dict(
                    checkpoint_dict, artifact_path=os.path.splitext(os.path.basename(checkpoint_path))[0]
                )
        ...
        coco_eval_bbox = test_stats.pop("coco_eval_bbox")
        test_stats["ap_0.50_det_100"] = coco_eval_bbox[1]
        test_stats["ar_0.50_det_100"] = coco_eval_bbox[-4]
        mlflow.log_metrics({f"train_{k}": v for k, v in train_stats.items()})
        mlflow.log_metrics({f"test_{k}": v for k, v in test_stats.items()})
